...
import type { Group } from "three"; import { Canvas, useFrame } from "@react-three/fiber";
...
export const Model = () => {
    const { nodes, materials } = useGLTF("/3dModels/Neurons.glb") as GLTFResult;;
    const groupRef = useRef<Group>(null);
    useFrame((state) => {
        const t = state.clock.getElapsedTime()
        groupRef.current!.rotation.x = THREE.MathUtils.lerp(groupRef.current!.rotation.x, Math.cos(t / 2) / 20 + 0.25, 0.1)
        groupRef.current!.rotation.y = THREE.MathUtils.lerp(groupRef.current!.rotation.y, Math.sin(t / 2) / 20, 0.1)
...
export const Neurons = () => {
    return (
        <Canvas camera={{ position: [2, 0, 3], fov: 55 }}>
            <pointLight position={[10, 10, 10]} intensity={1.5} />
            <Suspense fallback={null}>
                <group rotation={[0, Math.PI, 0]} position={[0, 1, 0]} scale={20}>
                    <Model />
                </group>
            ...
            <OrbitControls minPolarAngle={Math.PI / 2.2} maxPolarAngle={Math.PI / 2}  />
        </Canvas>
    )
}
useGLTF.preload("/3dModels/Neurons.glb");