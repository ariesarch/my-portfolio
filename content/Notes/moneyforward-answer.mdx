---
title: Interview Coding Challenge
...
interface ReservationServiceInterface
{
    ...
    public function stopReservationPeriod(string $restaurantId, int $suspend_date, string $timeRange): void;
}
...
class ReservationService implements ReservationServiceInterface
{
    ...
    public function stopReservationPeriod(string $restaurantId, int $suspend_date, string $timeRange): void
    {
        $this->system->stopReservationPeriod($restaurantId, $suspend_date, $timeRange);
    }
}
...
class Restaurant
{
    ...
    /**
     *
     * @var  array<array<array<string>>> $suspendedTimeRanges
     */
    ...
    /**
     *
     * @return  array<array<array<string>>> $suspendedTimeRanges
     */
    ...
    public function isWithinTemporyClose(int $reserveDayIndex,string $reserveTime): bool 
    {
        ...
        foreach ($this->suspendedTimeRanges[$reserveDayIndex] as $range) {
            $closingTimeStart = strtotime($range['start']);
            $closingTime = strtotime($range['end']);
    ...
    public function stopReservationPeriod(string $restaurantId, int $suspend_date, string $timeRange): void
    {
        // Temporarily stop receiving reservation requests for the specified suspend_date and time range
        $this->reservationService->stopReservationPeriod($restaurantId,$suspend_date,$timeRange);
    }
...
function prepareBusinessHour(int $startPoint, int $endPoint, array $payload): array
{
    $businessHours = [];
    for ($i = $startPoint; $i <= $endPoint; $i++) {
        $hours = explode(" ", (string)$payload[$i]);
        $hourPerDay = [];
...
function main(array $inputArray): void
{
    ...
    foreach ($inputArray as $value) {
        if (strpos($value, 'contact@') !== false ) { // Check if the restaurant count is less than 2
            ...
            // $restaurantCount++; // Increment the restaurant count
        } else {
            $modified_array[$temp_key][] = $value;
        }
    }
    $system = new System();
